...
public partial class MetadataForm : Form
{
    ...
    private readonly Label nometadatafoundlabel = new()
        { Text = "No metadata found! :(", Location = new Point(10, 10), AutoSize = true };

    private LinkLabel nometadatafoundlinklabel =
        new() { Text = "Try to reload it?", Location = new Point(10, 20), AutoSize = true };
    ...
    private void AddLabel(string? text, int pos, int x, Color? color = null)
    {
        ...
        Controls.Add(new Label
        {
            ...
            Location = new Point(x,
                pos == 1 ? 15 : Controls[Controls.Count - 1].Location.Y + Controls[Controls.Count - 1].Size.Height + 5),
            AutoSize = true, ForeColor = (Color)color
        });
    }
    ...
    private void processproperties(object thing, int meta1 = 20, int meta2 = 30, int graduality = 10, int overflow = 3)
    {
        ...
        foreach (var property in properties.Where(x => x.CanRead && x.GetGetMethod()?.GetParameters().Length == 0)
            ...
            try
            {
                ...
                AddLabel(property.Name + ":", meta1, IsPartOfImplementation(property) ? Meta1 : Meta1B);
