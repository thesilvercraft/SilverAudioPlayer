...
public class Preferences
{
    ...
    [Comment(
        "Colour of progress bar The byte-ordering of the 32-bit ARGB value is AARRGGBB. The most significant byte (MSB), represented by AA, is the alpha component value. The second, third, and fourth bytes, represented by RR, GG, and BB, respectively, are the color components red, green, and blue, respectively. https://docs.microsoft.com/en-us/dotnet/api/system.drawing.color.fromargb?view=net-6.0")]
    ...
    [Comment(
        "Should the player read the metadata of files when they are added to the queue? also enables sorting by track number")]
    public bool FillMetadataOfLoadedFilesOnLoad { get; set; } = true;

    [Comment(
        "Should the player read args.txt so that it can load files from other instances? (like vlc does but worse)")]
    public bool AutoMagicallyLoadFromArgstxt { get; set; } = true;

    [Comment("Should the player react to media controls")]
    public bool HandleMediaControls { get; set; } =
