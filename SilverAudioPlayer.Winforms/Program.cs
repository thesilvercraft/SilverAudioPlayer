...
public class App
{
    ...
    private static readonly string[] AssociatedFileTypes =
        { ".mp3", ".aif", ".aiff", ".flac", ".wav", ".ogg", ".midi", ".mid" };
    ...
    private void DoContainerShit()
    {
        ...
        Task.Run(async () =>
        {
            foreach (var playprovider in frm1.Logic.PlayProviders) await playprovider?.OnStartup();
        });
    ...
    public static bool ShortCuts()
    {
        ...
        IWshRuntimeLibrary.IWshShortcut shortcut = wsh.CreateShortcut(
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                "Microsoft\\Windows\\SendTo\\SilverAudioPlayer (Play).lnk")) as IWshRuntimeLibrary.IWshShortcut;
    ...
    public static bool RemoveShortCuts()
    {
        ...
        var f = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "Microsoft\\Windows\\SendTo\\SilverAudioPlayer (Play).lnk");
    ...
    {
        if (string.IsNullOrEmpty((string?)Registry.GetValue("HKEY_CLASSES_ROOT\\SilverAudioPlayer", string.Empty,
                string.Empty)))
        {
            Registry.SetValue("HKEY_CURRENT_USER\\Software\\Classes\\SilverAudioPlayer", "", "Audio File");
            Registry.SetValue("HKEY_CURRENT_USER\\Software\\Classes\\SilverAudioPlayer", "FriendlyTypeName",
                "AudioPlayerZ Audio File");
    ...
    public void Run(string?[]? args)
    {
        ...
        if (File.Exists(Path.Combine(AppContext.BaseDirectory, "preferences.xml")))
            File.Move(Path.Combine(AppContext.BaseDirectory, "preferences.xml"),
                Path.Combine(AppContext.BaseDirectory, "settings\\", "silveraudioplayer.winforms.preferences.xml"));
...
#endif
        AutoUpdate au = new("https://silverwebsiterepo.pages.dev/silveraudioplayer/releases", "SilverAudioPlayer",
        ...
        if (mutex.WaitOne(TimeSpan.FromSeconds(2), true))
        {
            ...
            if (args?.Any(x => string.IsNullOrEmpty(x)) == false)
            {
                ...
                if (args.Length == 1 && args[0] == "--removereg")
                {
                    RemoveFromReg();
                    return;
                }

                if (args.Length == 1 && args[0] == "--shortcuts")
                {
                    ShortCuts();
                    return;
                }

                if (args.Length == 1 && args[0] == "--rshortcuts")
                {
                    RemoveShortCuts();
                    return;
                }
