...
namespace NAudio.Flac
{
    ...
    public class FlacReader : WaveStream, IDisposable
    {
            ...
            Action<FlacPreScanFinishedEventArgs> onscanFinished)
        {
            ...
            {
                //read metadata
                System.Collections.Generic.List<FlacMetadata> metadata = FlacMetadata.ReadAllMetadataFromStream(stream).ToList();
                ...
                if (!(metadata.First(x => x.MetaDataType == FlacMetaDataType.StreamInfo) is FlacMetadataStreamInfo streamInfo))
                {
