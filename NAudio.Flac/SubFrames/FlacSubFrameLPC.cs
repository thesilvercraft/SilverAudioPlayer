...
namespace NAudio.Flac.SubFrames
{
    ...
    internal sealed partial class FlacSubFrameLPC : FlacSubFrameBase
    {
        ...
        public unsafe FlacSubFrameLPC(FlacBitReader reader, FlacFrameHeader header, FlacSubFrameData data,
            int bitsPerSample, int order)
            : base(header)
        {
            var warmup = new int[order];
            for (var i = 0; i < order; i++) warmup[i] = data.ResidualBuffer[i] = reader.ReadBitsSigned(bitsPerSample);
            ...
            var q = new int[order];
            for (var i = 0; i < order; i++) q[i] = reader.ReadBitsSigned(coefPrecision);

            //decode the residual
            var residual = new FlacResidual(reader, header, data, order);
            for (var i = 0; i < order; i++) data.DestinationBuffer[i] = data.ResidualBuffer[i];
        ...
        /// <summary>
        ///     Copied from http://stackoverflow.com/questions/8970101/whats-the-quickest-way-to-compute-log2-of-an-integer-in-c
        ///     14.01.2015
        /// </summary>
