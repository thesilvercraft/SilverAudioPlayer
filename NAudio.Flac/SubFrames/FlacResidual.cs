...
namespace NAudio.Flac.SubFrames
{
    internal class FlacResidual
    {
        ...
        public FlacResidual(FlacBitReader reader, FlacFrameHeader header, FlacSubFrameData data, int order)
        {
            ...
            if (codingMethod == FlacResidualCodingMethod.PartitionedRice ||
                codingMethod == FlacResidualCodingMethod.PartitionedRice2)
            {
                var partitionOrder =
                    (int)reader.ReadBits(
                        4); //"Partition order." see https://xiph.org/flac/format.html#partitioned_rice and https://xiph.org/flac/format.html#partitioned_rice2
